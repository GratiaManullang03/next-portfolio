		...
		function animate() {
			...
			for (let i = 0; i < cols; i++) {
				for (let j = 0; j < rows; j++) {
					...
					const nR = (Math.sin(i * 0.04 + tR) + Math.cos(j * 0.03 + tR)) / 2;
					const nB = (Math.sin(i * 0.05 + tB) + Math.cos(j * 0.04 + tB)) / 2;
					...
					ripples.forEach((rip) => {
						const dToRipple = Math.hypot(centerX - rip.x, centerY - rip.y);
						const ringWidth = 60;
						if (Math.abs(dToRipple - rip.radius) < ringWidth) {
							let rInt = 1 - Math.abs(dToRipple - rip.radius) / ringWidth;
